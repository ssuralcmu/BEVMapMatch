...
def train_model(rank, world_size, num_epochs, model, criterion, optimizer, 
    ...
    with torch.no_grad():
        ...
        for stitched, basemap, labels, stitched_img_path, basemap_img_path, metas_path in pbar:
            ...
            results.append({
                ...
                "basemap_img_path": basemap_img_path,
                "metas_path": metas_path,
                "predictions": predictions.cpu().numpy().tolist(),
                "labels": labels.cpu().numpy().tolist(),
                "outputs": outputs.cpu().numpy().tolist(),
                "iou_percentage": float(iou_percentage)
            })
    ...
    if rank == 0:
        print(f"Val Loss: {val_loss:.4f}, Top-3 Val Acc: {val_acc:.2%}, Val IoU: {val_iou:.2f}")
        with open ("results_"+unique_name+".json", "w") as f:
...
def main():
    ...
    parser.add_argument('--version', type=str, default="8_WeightedBCELoss")
    ...
    # train_model(0, world_size, args.num_epochs, model, criterion, optimizer,
    #           train_dataset, val_dataset, args.batch_size, args.lr,
    #           args.version, args.train_fraction, args.checkpoint, args.seed)
