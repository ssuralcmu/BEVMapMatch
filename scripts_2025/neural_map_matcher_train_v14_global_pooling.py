...
class GridClassifier(nn.Module):
    def __init__(self):
        ...
        # Feature extractor (using ResNet-18 excluding the final two layers)
        resnet = models.resnet18(pretrained=True)
        self.feature_extractor = nn.Sequential(*list(resnet.children())[:-2])
        
        # 3x3 processing: pool basemap feature maps to a 10x10 grid
        self.grid_pool = nn.AdaptiveAvgPool2d((10, 10))

        self.dropout_fc = nn.Dropout(p=0.5)
        # Final classification layer (outputting 100 grid cells)
    ...
    def forward(self, stitched, basemap):
        ...
        # Pool basemap features to a 10x10 grid
        grid = self.grid_pool(basemap_feat)  # (B,512,10,10)
        
        # Unfold the grid and average over the local 3x3 patch
        grid_patches = F.unfold(grid, kernel_size=3, padding=1)  # (B,512*9,100)
        grid_patches = grid_patches.view(-1, 512, 9, 100).mean(dim=2)  # (B,512,100)
        
        # grid_patches: (B,512,100)
        # print("Grid patches shape:", grid_patches.shape)
        grid_patches = grid_patches.permute(0, 2, 1).contiguous()  # (B,100,512)
        # print("Grid patches permuted shape:", grid_patches.shape)
        outputs = self.fc(self.dropout_fc(grid_patches)).squeeze(-1)  # (B,100)   
...
def main():
    ...
    parser.add_argument('--version', type=str, default="14_GlobalPooling_DiceLoss")
