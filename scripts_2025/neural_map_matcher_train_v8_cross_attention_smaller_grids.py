...
class MapDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            max_i = min(24, grid_x + 2)
            min_j = max(0, grid_y - 1)
            max_j = min(24, grid_y + 2)
            ...
            return torch.zeros(3, 100, 100), torch.zeros(3, 1000, 1000), torch.zeros(1600), "", "", ""
...
class GridClassifier(nn.Module):
    def __init__(self):
        ...
        # 3x3 processing
        self.grid_pool = nn.AdaptiveAvgPool2d((40, 40))
        ...
        # Positional encoding
        self.pos_embed = nn.Parameter(torch.randn(1600, 512)*0.02)
        
        # 3x3 attention mask
        self.register_buffer('attn_mask', self.create_local_mask())
        
        self.fc = nn.Linear(512, 1600) 
    ...
    def create_local_mask(self):
        ...
        return torch.zeros(1, 1600, dtype=torch.bool)  # No masking
    ...
    def forward(self, stitched, basemap):
        ...
        grid_3x3 = grid_3x3.view(-1, 512, 9, 1600).mean(dim=2)  # (B,512,100)
    ...
    for epoch in range(start_epoch, num_epochs):
        try:
            ...
            for stitched, basemap, labels, stitched_img_path, basemap_img_path, metas_path in pbar:
                ...
                train_top1_correct = (labels.gather(1, top1) > 0).sum().item()
            ...
            losses["train"].append([epoch, train_loss, train_acc, train_top1_accuracy, train_iou])
            ...
            if epoch % 1 == 0 or epoch==start_epoch:
                ...
                with torch.no_grad():
                    ...
                    for stitched, basemap, labels, stitched_img_path, basemap_img_path, metas_path in pbar:
                        ...
                        _, top1 = torch.topk(outputs, 1, dim=1)
                        train_top1_correct = (labels.gather(1, top1) > 0).sum().item()
                        val_total_top1_correct += train_top1_correct
                ...
                losses["val"].append([epoch, val_loss, val_acc, val_top1_accuracy, val_iou])
            ...
            if rank == 0:
                print(f"Epoch [{epoch+1}/{num_epochs}], Train Loss: {train_loss:.4f}, Top-3 Train Acc: {train_acc:.2%}, Top-1 Train Acc: {train_top1_accuracy:.2f}, Val Loss: {val_loss:.4f}, Top-3 Val Acc: {val_acc:.2%}, Top-1 Val Acc: {val_top1_accuracy:.2f}, Train IoU: {train_iou:.2f}, Val IoU: {val_iou:.2f}")
...
def main():
    ...
    parser.add_argument('--version', type=str, default="8_BCELoss_SmallGrids")
