...
class LocationModel(nn.Module):
    def __init__(self):
        super(LocationModel, self).__init__()
        # Convolutional layers for stitched image (50x50)
        self.conv_stitched = nn.Sequential(
        ...
        # Convolutional layers for basemap (500x500)
        self.conv_basemap = nn.Sequential(
            nn.Conv2d(3, 32, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.MaxPool2d(2, 2),
            nn.Conv2d(32, 64, kernel_size=3, padding=1),
        ...
        self.conv_combined = nn.Sequential(
            nn.Conv2d(80, 128, kernel_size=3, padding=1),  # Changed from 128 to 80
            nn.ReLU(),
            nn.MaxPool2d(2, 2),
    ...
    def forward(self, stitched, basemap):
        # Process stitched image (50x50)
        x_stitched = self.conv_stitched(stitched)  # Output: (64, 50, 50)
        
        # Process basemap (500x500)
        x_basemap = self.conv_basemap(basemap)  # Output: (64, 125, 125)


        # print("stitched shape: ", x_stitched.shape)
        # print("basemap shape: ", x_basemap.shape)
        
        # Perform cross-correlation to find the best match
        correlation = F.conv2d(x_basemap, x_stitched)

        correlation = F.interpolate(correlation, size=x_basemap.shape[2:], mode='bilinear', align_corners=False)
        # print("correlation shape: ", correlation.shape)
        
        # Combine features
        combined = torch.cat((correlation, x_basemap), dim=1)
...
def create_dataloader(rank, world_size, dataset, batch_size=16, num_workers=4):
...
def train_model(rank, world_size, num_epochs, model, criterion, optimizer, train_dataset, val_dataset):
    ...
    for epoch in range(num_epochs):
        ...
        if rank == 0 and train_loss < best_train_loss:
            ...
            torch.save(best_train_model_state, 'best_map_location_model_train.pth')
        ...
        if epoch % 5 == 0:
            ...
            if rank == 0 and val_loss < best_val_loss:
                ...
                torch.save(best_model_state, 'best_map_location_model_val.pth')
