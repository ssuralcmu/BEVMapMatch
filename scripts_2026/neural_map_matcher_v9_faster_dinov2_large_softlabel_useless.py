...
SOFT_LABEL_SIGMA = 1.0 / np.sqrt(2.0 * np.log(2.0))
SOFT_LABEL_CUTOFF = 1.0
...
class MapDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if target_block != 1:
            raise ValueError("Soft labels assume TARGET_BLOCK=1.")

        y_idx, x_idx = torch.meshgrid(
            torch.arange(grid_dim, dtype=torch.float32),
            torch.arange(grid_dim, dtype=torch.float32),
            indexing="ij",
        )
        dist = torch.sqrt((y_idx - i0) ** 2 + (x_idx - j0) ** 2)
        soft_labels = torch.exp(-(dist ** 2) / (2.0 * SOFT_LABEL_SIGMA ** 2))
        soft_labels = torch.where(
            dist <= SOFT_LABEL_CUTOFF,
            soft_labels,
            torch.zeros_like(soft_labels),
        )
        grid_label = soft_labels
...
def main():
    ...
    parser.add_argument('--version', type=str, default="9_dinov2_softlabel")
