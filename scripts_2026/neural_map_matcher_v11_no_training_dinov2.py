...
class DINOv2Embedder(nn.Module):
    ...
    Returns L2-normalized CLS embedding (B, D).
    """
    ...
    def forward(self, x):
        ...
        # last_hidden_state: (B, 1+N, D); CLS token is index 0
...
def run_inference_resnet_retrieval(
    ...
    for stitched, basemap, labels, stitched_img_path, basemap_img_path, metas_path in pbar:
        ...
        # ensure DINOv2-friendly size
        stitched_in = F.interpolate(stitched, size=(224, 224), mode="bilinear", align_corners=False)
        with torch.cuda.amp.autocast(enabled=amp and (device.type == "cuda")):
            stitched_emb = embedder(stitched_in)
        ...
        # resize cells to ViT input size (224x224)
        basemap_cells_flat = F.interpolate(basemap_cells_flat, size=(224, 224), mode="bilinear", align_corners=False)

        with torch.cuda.amp.autocast(enabled=amp and (device.type == "cuda")):
            cell_emb_flat = embedder(basemap_cells_flat)
...
def main():
    ...
    parser.add_argument('--version', type=str, default="11_dino")
    ...
    if args.mode == "infer":
        ...
        if args.infer_method == "retrieval":
            ...
            embedder = DINOv2Embedder(model_name="facebook/dinov2-large")
