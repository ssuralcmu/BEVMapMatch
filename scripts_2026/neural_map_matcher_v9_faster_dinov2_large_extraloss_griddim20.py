...
class GridClassifier(nn.Module):
    def __init__(self):
        ...
        self.grid_pool = nn.AdaptiveAvgPool2d((GRID_DIM, GRID_DIM))
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, GRID_DIM*GRID_DIM, self.embed_dim))
        nn.init.trunc_normal_(self.pos_embed, std=0.02)

        # No mask needed if you want full attention
        self.attn_mask = None

        self.fc = nn.Linear(self.embed_dim, GRID_DIM*GRID_DIM)
    ...
    def forward(self, stitched, basemap):
        ...
        grid_3x3 = F.unfold(grid, kernel_size=3, padding=1)  # (B,D*9,GRID_DIM*GRID_DIM)
        grid_3x3 = grid_3x3.view(grid_3x3.size(0), self.embed_dim, 9, GRID_DIM*GRID_DIM).mean(dim=2)  # (B,D,GRID_DIM*GRID_DIM)

        # Attention inputs
        query = F.adaptive_avg_pool2d(stitched_feat, (1, 1)).flatten(1)      # (B,512)
        query = query.unsqueeze(1)                                           # (B,1,512)

        key_value = grid_3x3.permute(0, 2, 1)                                 # (B,GRID_DIM*GRID_DIM,D)
        key = value = key_value + self.pos_embed                              # (B,GRID_DIM*GRID_DIM,D)

        # Cross-attention
        attn_out, _ = self.cross_attn(
            query=query,   # (B,1,D)
            key=key,       # (B,GRID_DIM*GRID_DIM,D)
            value=value,   # (B,GRID_DIM*GRID_DIM,D)
            attn_mask=self.attn_mask
        )

        scores = self.fc(attn_out.squeeze(1))  # (B,GRID_DIM*GRID_DIM)
        return scores
...
def visualize_pred_gt_grid(pred_mask_10x10, gt_mask_10x10, save_path):
    ...
    # Build RGB image
    img = np.ones((GRID_DIM, GRID_DIM, 3), dtype=np.float32)  # white background
    ...
    plt.xticks(range(GRID_DIM))
    plt.yticks(range(GRID_DIM))
        ...
        for stitched, basemap, labels, stitched_img_path, basemap_img_path, metas_path in pbar:
            ...
            if viz:
                for b in range(labels.size(0)):
                    gt_10x10 = labels[b].view(GRID_DIM, GRID_DIM)
                    pred_10x10 = preds[b].view(GRID_DIM, GRID_DIM)
...
def main():
    ...
    parser.add_argument('--version', type=str, default="9_dinov2_extraloss_wt0point1_griddim20")
