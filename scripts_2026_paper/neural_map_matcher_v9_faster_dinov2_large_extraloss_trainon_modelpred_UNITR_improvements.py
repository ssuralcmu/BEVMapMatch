...
class TransformerAddOn(nn.Module):
    """
    Minimal transformer-style addition with:
      - pre-norm cross-attn (no dropout)
      - residual
      - pre-norm MLP (GELU) + residual (no dropout)

    This is the smallest "Transformer block" you can add without changing slots, head, etc.
    """
    ...
    def forward(self, q, kv, attn_mask=None):
        # Cross-attn (pre-norm) + residual
        qn = self.ln_q1(q)
        kvn = self.ln_kv(kv)
        attn_out, _ = self.attn(qn, kvn, kvn, attn_mask=attn_mask, need_weights=False)
        q = q + attn_out

        # MLP (pre-norm) + residual
        q = q + self.mlp(self.ln_q2(q))
        return q


class GridClassifierV2(nn.Module):
    """
    V1 + ONLY a transformer block addition. No dropout. Same slots (B,1,D).
    - Same DINO backbone
    - Same basemap->grid pooling
    - Same unfold+mean neighborhood (unchanged)
    - Same pos_embed (unchanged)
    - Same fc head (unchanged)

    The ONLY difference: instead of directly using cross_attn output,
    we run one TransformerAddOn block (cross-attn + MLP with residuals).
    """
    def __init__(self):
        super().__init__()
        ...
        # Keep the original attention module (no dropout)
        ...
        # Keep original positional embedding
        self.pos_embed = nn.Parameter(torch.zeros(1, 100, self.embed_dim))
        nn.init.trunc_normal_(self.pos_embed, std=0.02)

        self.attn_mask = None

        # NEW: transformer-style addition (one block)
        self.tx = TransformerAddOn(dim=self.embed_dim, num_heads=8, mlp_ratio=4.0)

        # Same head
        self.fc = nn.Linear(self.embed_dim, 100)

        # Optional: start near-V1 behavior by initializing the MLP to ~0
        # so early training resembles your original model.
        nn.init.zeros_(self.tx.mlp[-1].weight)
        nn.init.zeros_(self.tx.mlp[-1].bias)

    def forward(self, stitched, basemap):
        # Feature extraction
        stitched_feat = self.feature_extractor(stitched)   # (B,D,h,w)
        basemap_feat  = self.feature_extractor(basemap)    # (B,D,h,w)

        # Basemap -> 10x10 grid
        grid = self.grid_pool(basemap_feat)                # (B,D,10,10)

        # Same neighborhood aggregation as V1
        ...
        # Original cross-attn (V1 behavior)
        attn_out, _ = self.cross_attn(
            query=query, key=kv, value=kv, attn_mask=self.attn_mask, need_weights=False
        )  # (B,1,D)

        # ONLY addition: run a transformer block that attends to kv and refines q
        # We pass attn_out as q (so it is truly "addition on top of V1")
        q_refined = self.tx(attn_out, kv, attn_mask=self.attn_mask)  # (B,1,D)

        scores = self.fc(q_refined.squeeze(1))  # (B,100)
        return scores
...
def main():
    ...
    parser.add_argument('--version', type=str, default="improve")
