...
def main():
    ...
    parser.add_argument('--max_infer_samples', type=int, default=0, help="Limit inference to first N samples (0 means all).")
    ...
    if args.mode == "infer":
        ...
        if args.max_infer_samples and args.max_infer_samples > 0:
            limit = min(len(infer_dataset), args.max_infer_samples)
            infer_dataset = torch.utils.data.Subset(infer_dataset, range(limit))
            print(f'Applying max_infer_samples={args.max_infer_samples}; using {limit} samples')
        run_inference(
            model=model,
