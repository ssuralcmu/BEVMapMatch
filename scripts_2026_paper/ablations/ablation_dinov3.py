...
def remap_dinov3_checkpoint_keys(state_dict):
    """
    Remap older/newer DINOv3 HF key names to current model key names.
    Handles cases like:
      feature_extractor.model.cls_token -> feature_extractor.model.embeddings.cls_token
    """
    remapped = {}
    for k, v in state_dict.items():
        ...
        # DINOv3/HF internal naming changes
        # Common top-level tokens moved under .embeddings
        new_k = new_k.replace("feature_extractor.model.cls_token", "feature_extractor.model.embeddings.cls_token")
        new_k = new_k.replace("feature_extractor.model.mask_token", "feature_extractor.model.embeddings.mask_token")
        new_k = new_k.replace("feature_extractor.model.position_embeddings", "feature_extractor.model.embeddings.position_embeddings")
        new_k = new_k.replace("feature_extractor.model.patch_embeddings.", "feature_extractor.model.embeddings.patch_embeddings.")
...
def main():
    ...
    parser.add_argument('--version', type=str, default="dinov3")
