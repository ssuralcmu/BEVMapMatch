...
def build_condition_subset(dataset, subset_name, sample_flag_lookup):
    ...
    keep_indices = []
    for idx, entry in enumerate(dataset.sample_entries):
        metas_path = os.path.join(dataset.metas_folder, entry["metas_file"])
        metas = np.load(metas_path, allow_pickle=True).item()
...
def main():
    ...
    parser.add_argument('--version', type=str, default="4frames_together")
