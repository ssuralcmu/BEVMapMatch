...
GENERATED_MAP_INPUT_SIZE = 224
BASEMAP_RESIZE_SIZE = BASEMAP_INPUT_SIZE * 2
...
def main():
    ...
    parser.add_argument('--version', type=str, default="larger_input")
    ...
    transform_base = transforms.Compose([
        transforms.Resize((BASEMAP_RESIZE_SIZE, BASEMAP_RESIZE_SIZE)),
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    ])
    transform_gen = transforms.Compose([
        transforms.Resize((GENERATED_MAP_INPUT_SIZE, GENERATED_MAP_INPUT_SIZE)),
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    ])
